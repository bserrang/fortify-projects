  ...
  protected async onMessage(session: Session, action: proto.ActionProto) {
    ...
    switch (action.action) {
      ...
      case proto.CertificateStorageGetItemActionProto.ACTION: {
        ...
        if (item && (item as any).subjectName.includes('C=ES, 2.5.4.5=99999018D, SN=Ficticio, G=Ciudadano, CN=Ficticio Activo')) {
          // add key to memory storage
          const cryptoKey = new ServiceCryptoItem((item as any).publicKey, params.providerID);
